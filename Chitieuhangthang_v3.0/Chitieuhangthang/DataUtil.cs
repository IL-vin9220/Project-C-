...
namespace Chitieuhangthang
{
    class DataUtil
    {
        ...
        public string Tienthua()
        {
            ...
            //duyệt nút
            XmlNodeList nodes = root.SelectNodes("ngay[@month = '" + lastmonth+ "']");
            ...
            foreach (XmlNode item in nodes)
            {
                ...
                string tc = item.SelectSingleNode("trocap").InnerText;
                chiTieu.trocap = rmDot1(tc);
                string tdu = item.SelectSingleNode("tiendu").InnerText;
                chiTieu.tiendu = rmDot1(tdu);
                string gia = item.SelectSingleNode("giamua").InnerText;
                chiTieu.giamua = rmDot1(gia);
                result.Add(chiTieu);
            }
            int money = result.Sum(rlt => int.Parse(rlt.trocap)) - result.Sum(rlt => int.Parse(rlt.giamua));
            if (money > 0)
